...
export default function Layout() {
  ...
  return (
    <Row itemsCenter justifyBetween relative className="border-b px-4 py-2 w-full bg-white" tag={'header'} lgCol>
      ...
      <Row sm itemsCenter tag="nav" lgCol className={`max-lg:w-full max-lg:flex ${isVisible ? 'max-lg:flex' : 'max-lg:hidden'}`}>
        ...
        <Button
          tag={Link}
          href="/more/donate"
          className={`bg-gradient-to-br from-orange-400 to-red-600 border-orange-400 text-white hover:opacity-90 w-full ${pathname === '/more/donate' ? 'shadow-xl' : ''}`}
        >
          Donate
        </Button>
      </Row>
